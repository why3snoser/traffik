...
export function GoalShowcaseCard({ goal, totalUsd, fmtUsd, onDelete, onOpen }: GoalShowcaseCardProps) {
  ...
  return (
    <motion.button
      ...
      className="group relative w-full text-left overflow-hidden rounded-[26px] border border-white/10 bg-black/30 backdrop-blur-sm p-5 transition-colors hover:border-white/20 cursor-pointer"
    >
      {/* Ambient glow */}
      <motion.div
      ...
      <div className="relative flex items-center gap-5">
        ...
        <div className="relative shrink-0">
          ...
          <div
            ...
            style={{ background: `radial-gradient(circle, ${hexToRgba(color, 0.5)} 0%, transparent 70%)` }}
          />
          <div className="relative h-24 w-24 rounded-full border border-white/10 bg-black/30 flex items-center justify-center overflow-hidden">
            {goal.imageUrl ? (
              <img
                src={goal.imageUrl}
                alt={goal.title}
                className="w-full h-full object-cover"
                style={{ objectPosition: goal.imagePosition ?? 'center top', filter: 'brightness(0.85) saturate(0.7)' }}
                draggable={false}
              />
            ) : (
        ...
        <div className="flex-1 min-w-0">
          ...
          <h4 className="text-lg font-bold tracking-tight text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-500 truncate">
            {goal.title}
          </h4>
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between mb-1.5 text-xs">
              <span className="text-zinc-500">Прогрес</span>
      ...
      <button
        ...
        className="absolute top-2.5 right-2.5 w-7 h-7 rounded-lg bg-white/[0.05] border border-white/10 flex items-center justify-center text-zinc-500 opacity-0 group-hover:opacity-100 hover:text-red-400 transition-all"
      >
        <Trash2 size={12} />
      </button>
    </motion.button>
