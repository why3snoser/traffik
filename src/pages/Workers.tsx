  ...
  return (
    <div className="px-4 pt-6 pb-28">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-text">Воркеры</h1>
          <p className="text-text-muted text-sm mt-0.5">{workers.length} человек</p>
        </div>
        <button
          onClick={() => setShowAdd(true)}
          className="w-10 h-10 rounded-xl bg-accent flex items-center justify-center shadow-glow-sm"
        >
      ...
      {totalRub > 0 && (
        <div className="bg-gradient-to-r from-accent/15 to-success/10 border border-accent/20 rounded-2xl p-4 mb-5">
          <p className="text-text-muted text-xs mb-1">Общий профит со всех воркеров</p>
      ...
      {workers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          <div className="w-20 h-20 rounded-3xl bg-card border border-border flex items-center justify-center text-4xl">
            👥
          </div>
          <div className="text-center">
            <p className="text-text font-semibold">Нет воркеров</p>
            <p className="text-text-muted text-sm mt-1">Нажми + чтобы добавить</p>
          </div>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                {worker.totalProfit > 0 ? (
                  ...
                  <p className="text-text-muted text-xs mt-1">Нет профитов</p>
                )}
              </button>
            )
          })}
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center" onClick={() => setShowAdd(false)}>
          ...
          >
            ...
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-bold text-text">Новый воркер</h3>
              <button onClick={() => setShowAdd(false)} className="text-text-muted">
                <X size={18} />
              </button>
            </div>
