...
import { rubToUsd, usdToUah, fmtUsd, fmtUah, PROFIT_LABELS } from '@/types'
...
export default function WorkerDetail() {
  ...
  if (!worker) return <div className="p-8 text-text-muted">Не найдено</div>
  ...
  const handleDelete = () => {
    if (confirm(`Удалить воркера ${worker.name}?`)) {
  ...
  return (
    <div className="pb-28">
      ...
      <div className="px-4 pt-6 mb-5">
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            ...
            <p className="text-text-muted text-sm">{myAnketas.length} анкет</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          {[
            { label: 'Неделя', rub: stats.week },
            { label: 'Месяц', rub: stats.month },
            { label: 'Всего', rub: stats.total },
          ].map(({ label, rub }) => (
        ...
        {/* Add profit button */}
        <button
          onClick={() => navigate(`/workers/${id}/profit/new`)}
      ...
      <div className="flex gap-1 px-4 mb-4">
        {(['anketas', 'profits'] as const).map(t => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === t ? 'bg-accent text-white' : 'bg-card text-text-muted'}`}
          >
            {t === 'anketas' ? `Анкеты (${myAnketas.length})` : `Профиты (${myProfits.length})`}
          </button>
        ))}
      </div>
      ...
      <div className="px-4">
        {tab === 'anketas' && (
          <>
            <button
              onClick={() => navigate(`/workers/${id}/anketas/new`)}
            ...
            {myAnketas.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                ...
                <p className="text-text-muted text-sm">Нет анкет</p>
              </div>
            ) : (
              <div className="flex flex-col gap-2">
                {myAnketas.map(anketa => (
                  <button
                    key={anketa.id}
                    onClick={() => navigate(`/anketas/${anketa.id}`)}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-text-muted text-xs mt-0.5">
                        {anketa.cities.length} {anketa.cities.length === 1 ? 'город' : 'города'}
        ...
        {tab === 'profits' && (
          grouped.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-text-muted text-sm">Нет профитов</p>
            </div>
          ) : (
                    ...
                    {entries.map(entry => (
                      <div key={entry.id} className="bg-card border border-border rounded-2xl px-4 py-3">
                        ...
                        <div className="flex items-baseline gap-1 mt-0.5">
                          ...
                          <span className="text-text-muted text-xs">· вход: {entry.amount.toLocaleString('ru-RU')} ₽</span>
                        </div>
                      </div>
                    ))}
                  </div>
